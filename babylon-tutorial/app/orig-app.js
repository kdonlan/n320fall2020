...
    
                var forceDirection = BABYLON.Ray.CreateNewFromTo(camera.position, camera.getTarget()).direction;
                pickResult = pickedMesh.pick(scene.pointerX, scene.pointerY);
                pickResult.physicsImpostor.applyForce(
                                forceDir, 
                                pickResult.getAbsolutePosition()
                             );
            pickResult.pickedMesh.physicsImpostor.applyForce(
                pushDir, pickResult.pickedMesh.getAbsolutePosition()
                    );
